...
# NOTE: the original distribution is not really poisson distribution
# When you doubt about the original distribution, bootstrap will be your good friend
...
biden <- read_csv(here("data", "biden.csv"),col_types = cols())
```
...
p <- ggplot(biden, aes(x = age, y = biden)) +
  geom_point()
p + stat_smooth(method = 'lm', formula = biden ~ poly(age,4), size = 1) + theme_minimal()
# reference: https://stats.idre.ucla.edu/r/faq/how-can-i-explore-different-smooths-in-ggplot2/
# don't know why got error message: Computation failed in `stat_smooth()`: object 'age' not found
...
```{r}
t_statistics <- biden_lm_poly$coefficients / sqrt(diag(var_matrix))
tibble(t_statistics)
```
...
```{r}
tibble(tidy(biden_lm_poly)['statistic'], t_statistics)
```
