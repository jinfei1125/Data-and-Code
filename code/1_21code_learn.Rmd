...
```{r}
(train_mse <- augment(auto_lm, newdata = auto_train) %>% #augment = predict 
...
```{r}
set.seed(1234)
auto_cv10 <- vfold_cv(data = titanic, 
                      v = 10)
auto_cv10
auto_cv10$splits[[1]]
...
The two values are quite close. But, The error rate obtained from the 10-fold cross validation is lower than that of the LOOCV cross validation. 

Using the 10-fold cross validation, the variance would be decreased though there is a relatively smaller increase in the bias. As a result, the overall effect is that the error rate in the 10-fold cross validation would be decreased.

The LOOCV method is computationally expensive as the model has to be ﬁtted n times. However, the 10-fold cross validation performs better in this respect.
